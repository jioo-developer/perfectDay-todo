...
컴포넌트가 재 랜더링 될 때 변수나 리턴이 있는 모든 함수가 재 실행 되는데 이때 바뀌지 않는 값을 새로 불리오지<br />않고 저장해서 불러오는 것을 메모제이션 이라고 말함
...
<br /><br />
memorizeDay라는 변수에 useMemo를 연결해서 useMemo의 콜백 함수로 dayMemo를 실행하면<br />결론적으로 메모제이션이 됨.
